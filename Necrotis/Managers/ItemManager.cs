...
public class RequiredResourceList
{
    ...
    public void Add(string itemName, int amount, int quality = 0) => Requirements.Add(new Requirement
        { itemName = itemName, amount = amount, quality = quality });

    public void Add(string itemName, ConfigEntry<int> amountConfig, int quality = 0) => Requirements.Add(new Requirement
        { itemName = itemName, amountConfig = amountConfig, quality = quality });
}
...
public class ItemEffects
{
    ...
    public readonly List<string> HoldStartEffects = new();
...
public class Item
{
    ...
    public void AddStartEffect(string name) => ItemEffects.StartEffects.Add(name);
    public void AddHoldStartEffect(string name) => ItemEffects.HoldStartEffects.Add(name);
    public void AddTriggerEffect(string name) => ItemEffects.TriggerEffects.Add(name);
    public void AddTrailStartEffect(string name) => ItemEffects.TrailStartEffects.Add(name);
        ...
        SerializedRequirements upgradeRequirements)
    {
        if (ObjectDB.instance && activeRecipes.ContainsKey(this) &&
            activeRecipes[this].TryGetValue(recipeKey, out List<Recipe> recipes))
        {
            ...
            foreach (Item item in registeredItems.Where(i => i.configurability != Configurability.Disabled))
            {
                ...
                if ((item.configurability & Configurability.Trader) != 0)
                {
                    ...
                    ConfigEntry<T> traderConfig<T>(string name, T value, string desc)
                    {
                        ...
                        ConfigEntry<T> cfg = config(englishName, name, value,
    ...
    private void InitializeNewRegisteredItem()
    {
        foreach (KeyValuePair<string, ItemRecipe> kv in Recipes)
        {
            if (kv.Value.RecipeIsActive is { } enabledCfg)
            {
                ...
                {
                    if (ObjectDB.instance && activeRecipes.ContainsKey(this) &&
                        activeRecipes[this].TryGetValue(kv.Key, out List<Recipe> recipes))
                    {
                        foreach (Recipe recipe in recipes)
                        {
    ...
    internal static void AddItemEffects(ObjectDB __instance)
    {
        ...
        foreach (Item? item in registeredItems)
        {
            ...
            AddEffect(item.ItemEffects.HitTerrainEffects, ref component.m_itemData.m_shared.m_hitTerrainEffect);
            AddEffect(item.ItemEffects.BlockEffects, ref component.m_itemData.m_shared.m_blockEffect);
            AddEffect(item.ItemEffects.StartEffects, ref component.m_itemData.m_shared.m_startEffect);
    ...
    internal static void AddEffect(List<string> effects, ref EffectList list)
    {
        ...
        IEnumerable<CodeInstruction> instructionsEnumerable, ILGenerator ilg)
    {
        ...
        */
        ...
        for (int i = 0; i < instructions.Count; ++i)
        {
            ...
            if (inLoop)
            {
                if (instructions[i].Branches(out Label? target) &&
                ...
                foreach (Requirement req in reqs.Where(r => r.quality == quality))
                {
                    ...
                    if (int.TryParse(
                            GUILayout.TextField(amount.ToString(),
                                new GUIStyle(GUI.skin.textField) { fixedWidth = 40 }), out int newAmount) &&
